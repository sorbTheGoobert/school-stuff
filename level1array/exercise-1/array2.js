...
function displayNames(array) {
    let temp = [];
    array.forEach(function (element) { temp.push(`name: ${element.animal_name}, scientific: ${element.scientific_name}`) });
    return temp;
}
...
function lowCaseAnimal(array) {
    return array.map(function (element) { return `${element.animal_name}`.toLowerCase() });
}

let lowCaseAnimalNames = lowCaseAnimal(zooAnimals)
...
function filterLowPopAnimals(array) {
    return array.filter(function (element) { return element.population < 5 });
}

lowPopulationAnimals = filterLowPopAnimals(zooAnimals);
...
let population = findMost(zooAnimals);
